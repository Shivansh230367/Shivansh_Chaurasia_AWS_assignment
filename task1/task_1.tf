...
resource "aws_subnet" "public_a" {
  ...
  tags                    = { Name = "${var.resource_prefix}_public_a" }
}

resource "aws_subnet" "public_b" {
  vpc_id                  = aws_vpc.this.id
  cidr_block              = "10.0.2.0/24" # PUBLIC B
...
resource "aws_subnet" "private_a" {
  ...
  availability_zone = data.aws_availability_zones.available.names[0]
  tags              = { Name = "${var.resource_prefix}_private_a" }
}

resource "aws_subnet" "private_b" {
  vpc_id            = aws_vpc.this.id
  cidr_block        = "10.0.102.0/24" # PRIVATE B
  availability_zone = data.aws_availability_zones.available.names[1]
...
resource "aws_route_table" "public" {
  ...
  tags = { Name = "${var.resource_prefix}_public_rt" }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "${var.resource_prefix}_private_rt" }
}
